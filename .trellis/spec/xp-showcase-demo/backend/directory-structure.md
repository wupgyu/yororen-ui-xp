...
- **自绘 XP 标题条**（应用层窗口装饰，不在 55 个 renderer trait 内）：`TitlebarOptions { appears_transparent: true, .. }` 隐藏系统标题栏；标题条 div 用 `.window_control_area(WindowControlArea::Drag)`，最小化 / 最大化 / 关闭按钮分别用 `Min / Max / Close`——Windows 经 WM_NCHITTEST 原生执行拖拽与按钮动作，无需 on_click。标题条配色读 `xp.titlebar.*` / `xp.button.primary_*` / `action.danger.*` 主题路径。
