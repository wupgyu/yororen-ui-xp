//! XP (Luna) list renderers: `ListItem`, `Listbox`, `TreeItem`,
//! `Tree`, `Form`, `FormField`, `Table`, `VirtualList`,
//! `UniformVirtualList`.
//!
//! Rows rest transparent, hover to pale blue
...
use gpui::{
    ...
    KeyDownEvent, ParentElement, Pixels, SharedString, Stateful, StatefulInteractiveElement,
...
pub struct XpListItemRenderer;

// Inherent helpers — *not* part of the trait surface.
impl XpListItemRenderer {
    pub fn bg(&self, _: &ListItemRenderState, _theme: &Theme) -> Hsla {
        // XP list rows rest transparent — the white well (or
        // dialog surface) underneath provides the background.
    ...
    pub fn border_radius(&self, _: &ListItemRenderState, theme: &Theme) -> Pixels {
        px(theme
            .get_number("tokens.control.list_item.radius")
            .unwrap_or(XP_RADIUS as f64) as f32)
    }
}

impl ListItemRenderer for XpListItemRenderer {
    fn compose(
        &self,
...
pub struct XpListboxRenderer;

// Inherent helpers — *not* part of the trait surface. XP reuses
// the list_item control tokens so listbox rows stay in lockstep
// with standalone list items; the XP well styling (white sunken
// background, 1px bevel border) lives on the outer shell.
impl XpListboxRenderer {
    ...
    pub fn border_radius(&self, _: &ListboxRenderState, theme: &Theme) -> Pixels {
        px(theme
            .get_number("tokens.control.list_item.radius")
            .unwrap_or(XP_RADIUS as f64) as f32)
    }
...
impl ListboxRenderer for XpListboxRenderer {
    fn compose(
        &self,
        ...
        for (i, opt) in options.iter().enumerate() {
            ...
            let mut row: Stateful<Div> = gpui::div()
                .id(ElementId::Name(format!("xp-listbox-row-{}", i).into()))
                .flex()
                .items_center()
                .bg(row_bg)
                .text_color(row_fg)
...
pub struct XpTreeItemRenderer;

// Inherent helpers — *not* part of the trait surface.
impl XpTreeItemRenderer {
    pub fn bg(&self, _: &TreeItemRenderState, _theme: &Theme) -> Hsla {
        // Transparent resting state — the tree's white sunken
        // well shows through.
        gpui::hsla(0.0, 0.0, 0.0, 0.0)
    }
    ...
    pub fn border_radius(&self, _: &TreeItemRenderState, theme: &Theme) -> Pixels {
        px(theme
            .get_number("tokens.control.tree_item.radius")
            .unwrap_or(XP_RADIUS as f64) as f32)
    }
...
impl TreeItemRenderer for XpTreeItemRenderer {
    fn compose(
        &self,
...
pub struct XpFormRenderer;

// Inherent helpers — *not* part of the trait surface.
impl XpFormRenderer {
    pub fn gap(&self, _: &FormRenderState, theme: &Theme) -> Pixels {
        px(theme.get_number("tokens.control.form.gap").unwrap_or(8.0) as f32)
    }
    ...
    pub fn error_color(&self, _: &FormRenderState, theme: &Theme) -> Hsla {
        theme
            ...
            .unwrap_or(hsl_fallback(0xC75050))
    }
...
impl FormRenderer for XpFormRenderer {
...
pub struct XpVirtualListRenderer;

// Inherent helpers — *not* part of the trait surface.
impl XpVirtualListRenderer {
...
impl VirtualListRenderer for XpVirtualListRenderer {
    fn compose(
        &self,
        ...
        // pattern as the default renderer.
        let list_el =
            gpui::list(props.state, render_row).with_sizing_behavior(props.sizing_behavior);
        let inner = list_el.size_full().flex_grow().min_h_0();

        // The outer div is the XP sunken well: 1px bevel border,
        // white background, and overflow hidden so scrolled
        // content is clipped to the frame. `flex().flex_col()`
        // makes the inner list's `flex_grow()` work and lets any
        // sibling children (e.g. an info label added by the
        // caller) take their natural size at the top.
        //
...
pub struct XpUniformVirtualListRenderer;

// Inherent helpers — *not* part of the trait surface.
impl XpUniformVirtualListRenderer {
...
impl UniformVirtualListRenderer for XpUniformVirtualListRenderer {
    fn compose(
        &self,
        ...
        // RefCell trick the default renderer uses; XP only
        // changes the styling.
...
impl XpTreeRenderer {
    ...
    pub fn gap(&self, _state: &TreeRenderState, theme: &Theme) -> Pixels {
        // XP defaults to 0 row gap so adjacent tree items form a
        // continuous column, like the classic Explorer tree view.
...
impl TreeRenderer for XpTreeRenderer {
    fn compose(
        &self,
...
pub struct XpFormFieldRenderer;

// Inherent helpers — *not* part of the trait surface.
impl XpFormFieldRenderer {
...
impl FormFieldRenderer for XpFormFieldRenderer {
    fn compose(
        &self,
        ...
        // 1) Label row — XP keeps the required indicator as a
        //    plain `*` (no extra styling), matching the plain
        //    dialog-label look of classic forms.
        ...
        //    urgent and the status red already stands out
        //    against the dialog surface).
...
pub struct XpTableRenderer;

// Inherent helpers — *not* part of the trait surface.
impl XpTableRenderer {
    pub fn header_bg(&self, _state: &TableRenderState, theme: &Theme) -> Background {
        // The header row is the classic XP list-view header: a
        // raised column-header button wearing the button-face
        // gradient, not a solid fill.
        button_face(theme)
    }
    ...
    pub fn selected_bg(&self, _state: &TableRenderState, theme: &Theme) -> Hsla {
        // Luna blue selection — unmistakable, same as every
        // other XP list surface.
        xp_color(theme, "xp.selection.bg", selection_bg())
    }
...
impl TableRenderer for XpTableRenderer {
    fn compose(
        &self,
        ...
        // Header row — the classic XP list-view header at the
        // top of the table. A 1px bevel separator closes it off
        // from the body.
        ...
        // Body rows. Selected rows get the Luna blue selection;
        // other rows hover to pale blue. Rows are separated by
        // a thin bottom border in the bevel colour, like the
        // faint gridlines of a report-mode list view.
