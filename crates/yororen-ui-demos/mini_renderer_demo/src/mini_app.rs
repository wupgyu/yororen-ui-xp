...
impl Render for MiniApp {
    fn render(&mut self, _window: &mut Window, cx: &mut Context<Self>) -> impl IntoElement {
        ...
        // `RendererRegistry`).
        let _inc = button("inc", app).default_render(cx);
        let _reset = button("reset", app).default_render(cx);
        ...
        div()
            ...
            .flex_col()
            .gap_3()
            .child(label("title", "Mini renderer demo", app).default_render(cx))
            .child(
                label(
                    "blurb",
            ...
            .child(
                div()
                    .flex()
                    .gap_2()
                    .child(label("c", format!("Counter: {}", self.counter), app).default_render(cx))
                    .child(button("inc-btn", app).default_render(cx))
                    .child(button("reset-btn", app).default_render(cx)),
            )
    }
}
