...
pub fn render(_app: &mut GalleryApp, cx: &mut Context<GalleryApp>) -> impl IntoElement {
    ...
    let card_el = card("crd-1", cx)
        ...
        .w(px(220.))
        .child(
            label("card-title", cx.t("demo.surfaces.card_title"), cx)
                .strong(true)
                .render(cx),
        )
        .child(
            label("card-body", cx.t("demo.surfaces.card_body"), cx)
                .muted(true)
                .render(cx),
        )
        .child(
            button("card-btn", cx)
                .on_click(|_, _, _| {})
                .render(cx)
                .child(cx.t("demo.surfaces.card_action")),
        );
    ...
    let empty = empty_state("emp-1", cx)
        ...
        .w(px(280.))
        .child(
            label("es-extra", cx.t("demo.surfaces.empty_custom_child"), cx)
                .muted(true)
                .render(cx),
        );
    ...
    let img = image(
        "img-1",
        ImageSource::Resource("images/sample.png".into()),
        cx,
    )
    ...
    // --- keybinding_display ---
    let kbd_disp =
        keybinding_display("kbd-1", vec!["Ctrl".to_string(), "S".to_string()], cx).render(cx);
