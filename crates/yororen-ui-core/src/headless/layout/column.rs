//! `Column` — vertical flex container (`div().flex().flex_col()`).
//!
//! The primary layout primitive for stacking children top-to-bottom.
//! All spacing values (`gap`, `p`, `px`, `py`, `m`) resolve through
//! the active theme's `tokens.spacing.*` paths; see
//! [`super::types`] for the token mapping.
...
impl ColumnProps {
    ...
    pub fn render(self, cx: &App) -> Stateful<Div> {
        let theme = cx.theme();
        let mut el: Stateful<Div> = div().id(self.id).flex().flex_col();
